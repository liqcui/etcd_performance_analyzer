...
class PerformanceDataELT(utilityELT):
    ...
    def _identify_data_type(self, data: Dict[str, Any]) -> str:
        ...
        if 'data' in data and isinstance(data.get('data'), dict) and 'metrics' in data['data']:
            ...
            compact_defrag_metrics = ['debugging_mvcc_db_compaction_duration_sum_delta', 'debugging_mvcc_db_compaction_duration_sum',
                                    'disk_backend_defrag_duration_sum_rate', 'disk_backend_defrag_duration_sum',
                                    'vmstat_pgmajfault_rate', 'vmstat_pgmajfault_total']
            if any(metric in metrics_data for metric in compact_defrag_metrics):
                return 'compact_defrag'

        # Check direct metrics structure for compact defrag
        if 'metrics' in data:
            metrics_data = data['metrics']
            compact_defrag_metrics = ['debugging_mvcc_db_compaction_duration_sum_delta', 'debugging_mvcc_db_compaction_duration_sum',
                                    'disk_backend_defrag_duration_sum_rate', 'disk_backend_defrag_duration_sum',
    ...
    # Update the transform_to_dataframes method
    ...
    def generate_html_tables(self, dataframes: Dict[str, pd.DataFrame], data_type: str) -> Dict[str, str]:
        ...
        try:
            if data_type == 'cluster_info':
                ...
                return self.cluster_status_elt.generate_html_tables(dataframes)
            elif data_type == 'disk_io':
                return self.disk_io_elt.generate_html_tables(dataframes)
            elif data_type == 'wal_fsync':
                return self.wal_fsync_elt.generate_html_tables(dataframes)
            elif data_type == 'backend_commit':
                return self.backend_commit_elt.generate_html_tables(dataframes)
            elif data_type == 'general_info':
                return self.general_info_elt.generate_html_tables(dataframes)
            elif data_type == 'compact_defrag':
...
def json_to_html_table(json_data: Union[Dict[str, Any], str], compact: bool = True, two_column: bool = False) -> str:
    ...
    try:
        ...
        if result.get('summary'):
            ...
            # If summary is HTML (we generate from cluster info/status), pass through; otherwise escape
            if isinstance(summary_val, str) and summary_val.strip().startswith('<'):
                # Decode any unicode escapes like \u2705 to actual characters
                try:
                    util = utilityELT()
                ...
                # Decode unicode escapes before formatting
                try:
                    util = utilityELT()
                ...
                # Helper to HTML-escape text safely
                def _escape_html(s: str) -> str:
                    return s.replace('&', '&amp;').replace('<', '&lt;').replace('>', '&gt;')

                # Special formatting for Performance summaries: bold title phrase and new lines per bullet
                if 'Network I/O Performance' in summary_text or 'WAL Fsync Performance' in summary_text or '•' in summary_text:
                    parts = [p.strip() for p in summary_text.split('•')]
                    if parts:
                        first_line = _escape_html(parts[0])
                        # Bold only the title phrase if present
                        if 'Network I/O Performance' in first_line:
                            first_line = first_line.replace('Network I/O Performance', '<strong>Network I/O Performance</strong>')
                        if 'WAL Fsync Performance Analysis' in first_line:
                            first_line = first_line.replace('WAL Fsync Performance Analysis', '<strong>WAL Fsync Performance Analysis</strong>')
                        elif 'WAL Fsync Performance' in first_line:
                            first_line = first_line.replace('WAL Fsync Performance', '<strong>WAL Fsync Performance</strong>')
                        bullet_lines = [f"• {_escape_html(p)}" for p in parts[1:] if p]
                        formatted_summary = first_line
                        if bullet_lines:
                            formatted_summary += '<br>' + '<br>'.join(bullet_lines)
        ...
        # Add tables with priority ordering
        ...
        for priority_table in table_priority:
            if priority_table in html_tables:
                ...
                # And in the title mapping section, add:
                elif data_type == 'compact_defrag':
                    title_mapping = {
...
__all__ = [
    ...
    'process_network_io_json',  # Add this line
