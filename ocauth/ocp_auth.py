    ...
    async def _get_auth_details(self):
        ...
        try:
            ...
            if active_context:
                ...
                # Get token from configuration - try multiple approaches
                token = None
                
                # Method 1: Check api_key
                if hasattr(configuration, 'api_key') and configuration.api_key:
                    auth_token = configuration.api_key.get('authorization', '')
                    if auth_token.startswith('Bearer '):
                        token = auth_token[7:]  # Remove 'Bearer ' prefix
                    elif auth_token:
                        token = auth_token
                
                # Method 2: Check api_key_prefix (some configs store token here)
                if not token and hasattr(configuration, 'api_key_prefix'):
                    api_key_prefix = configuration.api_key_prefix or {}
                    if 'authorization' in api_key_prefix:
                        auth_value = api_key_prefix['authorization']
                        if auth_value and auth_value != 'Bearer':
                            # Sometimes the token is in api_key_prefix
                            if hasattr(configuration, 'api_key') and configuration.api_key:
                                potential_token = configuration.api_key.get('authorization', '')
                                if potential_token and not potential_token.startswith('Bearer'):
                                    token = potential_token
                
                # Method 3: Try to get token from the active context config
                if not token:
                    try:
                        # Load the kubeconfig dict to inspect token directly
                        config_dict = config.kube_config.Configuration.load_kube_config_file()
                        if config_dict:
                                        ...
                                        if user_name:
                                            ...
                                            for user in users:
                                                if user.get('name') == user_name:
                                                    ...
                                                    elif 'exec' in user_info:
                                                        # Token from exec command (like oc token)
                                                        self.logger.debug("Token from exec command detected")
                                                    break
                                        break
                    except Exception as e:
                        self.logger.debug(f"Could not parse kubeconfig for token: {e}")
                
                if token:
                    self.token = token
                    self.logger.info("Token extracted from kubeconfig")
                else:
                    self.logger.debug("No token found in kubeconfig, will try fallback methods")
                ...
                if hasattr(configuration, 'ssl_ca_cert') and configuration.ssl_ca_cert:
                    ...
                    self.logger.debug(f"CA cert path: {self.ca_cert_path}")
                    
        except Exception as e:
            self.logger.warning(f"Could not extract auth details from kubeconfig: {e}")
