        ...
        try:
            ...
            if data_rows:
                columns = [
                    ...
                    "pod_name", "avg_value", "max_value", "min_value", "latest_value",
                    "count_value", "unit", "status", "description", "total_data_points",
                    "created_at"
                ]
...
async def main():
    """Test the BackendCommitStorELT class"""
    import json
    
    # Sample test data (using the provided JSON structure)
    test_data = {
        "status": "success",
        "data": {
            "pods_metrics": {
                "disk_backend_commit_duration_seconds_p99": {
                    ...
                    "overall": {
                        "avg": 0.0010435830040295344,
                        "max": 0.0030699999999999825,
                        "min": 0.00099,
                        "latest": 0.0009928779069767442
                    },
                    "pods": {
                        "etcd-pod-1": {
                            "avg": 0.001029,
        ...
        "timestamp": "2025-09-20T02:28:53.094832+00:00",
        "duration": "1h"
    }
    
    # Test storage
    store = BackendCommitStorELT("test_backend_commit.duckdb")
    
    try:
        # Store data
        result = await store.store_backend_commit_metrics("test-123", test_data)
        print("Storage result:", json.dumps(result, indent=2, default=str))
        
        # Get summary
        summary = await store.get_backend_commit_summary("test-123")
        print("Summary result:", json.dumps(summary, indent=2, default=str))
        
        # Print summary table
        print_backend_commit_summary_tables(summary)
