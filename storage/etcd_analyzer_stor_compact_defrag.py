...
class CompactDefragStorELT(BaseStoreELT):
    ...
    async def _create_tables(self):
        ...
        try:
            ...
            CREATE TABLE IF NOT EXISTS {self.schema_name}.{self.summary_table} (
                ...
                min_raw DOUBLE,
                min_formatted VARCHAR,
                latest_raw DOUBLE,
                latest_formatted VARCHAR,
                count_value INTEGER,
                data_points INTEGER,
                interpretation JSON,
                created_at TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP
        ...
        try:
            ...
            for pod_name, pod_metrics in pods_data.items():
                ...
                row_data = (
                    ...
                    pod_metrics.get("max"),
                    pod_metrics.get("min"),
                    pod_metrics.get("latest"),
                    pod_metrics.get("count", 0),
                    unit,
                    "success",
                    json.dumps(pod_metrics, default=str)
                )
            ...
            columns = [
                ...
                "avg_value", "max_value", "min_value", "latest_value", "count_value",
                "unit", "status", "raw_data"
            ]
    ...
    async def _store_metric_summaries(self, testing_id: str, metric_summaries: Dict[str, Any], 
        ...
        try:
            ...
            for metric_name, summary_data in metric_summaries.items():
                if summary_data.get("status") == "success":
                    ...
                    row_data = (
                        ...
                        stats.get("min", {}).get("raw"),
                        stats.get("min", {}).get("formatted", ""),
                        stats.get("latest", {}).get("raw"),
                        stats.get("latest", {}).get("formatted", ""),
                        stats.get("count", 0),
            ...
            if data_rows:
                columns = [
                    "testing_id", "timestamp", "metric_name", "description", "unit", "status",
                    "avg_raw", "avg_formatted", "max_raw", "max_formatted", 
                    "min_raw", "min_formatted", "latest_raw", "latest_formatted",
    ...
    async def get_compact_defrag_summary(self, testing_id: str) -> Dict[str, Any]:
        ...
        try:
            # Get summary data
            summary_sql = f"""
            SELECT metric_name, description, unit, status, avg_raw, avg_formatted,
                   max_raw, max_formatted, min_raw, min_formatted, 
                   latest_raw, latest_formatted, count_value, data_points, 
            ...
            summary_columns = [
                "metric_name", "description", "unit", "status", "avg_raw", "avg_formatted",
                "max_raw", "max_formatted", "min_raw", "min_formatted",
                "latest_raw", "latest_formatted", "count_value", "data_points",
                "interpretation", "timestamp"
            ]
            ...
            for row in summary_rows:
                ...
                summaries[metric_name] = {
                    ...
                    "statistics": {
                        ...
                        "min": {"raw": row_dict["min_raw"], "formatted": row_dict["min_formatted"]},
                        "latest": {"raw": row_dict["latest_raw"], "formatted": row_dict["latest_formatted"]},
...
async def main():
    """Example usage of CompactDefragStorELT"""
    store = CompactDefragStorELT()
    
    try:
        # Example compact defrag data (similar to compact-defrag.json structure)
        sample_data = {
            "status": "success",
            "data": {
                "pods_metrics": {
                    "debugging_mvcc_db_compacted_keys": {
                        ...
                        "data": {
                            "pods": {
                                "etcd-pod-1": {"node": "node1", "avg": 1683218, "max": 1696551, "min": 1669768, "latest": 1687307, "count": 723},
                                "etcd-pod-2": {"node": "node2", "avg": 1683319, "max": 1692117, "min": 1674578, "latest": 1692117, "count": 723}
                            }
                        }
                    }
                },
                "metric_summaries": {
                    "debugging_mvcc_db_compacted_keys": {
                        ...
                        "description": "MVCC DB compacted keys",
                        "unit": "count",
                        "status": "success",
                        "statistics": {
                            "avg": {"raw": 1683218, "formatted": "1,683,218"},
                            "max": {"raw": 1696551, "formatted": "1,696,551"},
                            "min": {"raw": 1669768, "formatted": "1,669,768"},
                            "latest": {"raw": 1687307, "formatted": "1,687,307"},
                            "count": 723
                        },
                        "data_points": 723,
        ...
        result = await store.store_compact_defrag_metrics(testing_id, sample_data)
        print(f"Store result: {json.dumps(result, indent=2, default=str)}")
        
        # Test getting summary
        summary = await store.get_compact_defrag_summary(testing_id)
        print(f"Summary result: {json.dumps(summary, indent=2, default=str)}")
        
        # Print summary table
        print_compact_defrag_summary_tables(summary)
