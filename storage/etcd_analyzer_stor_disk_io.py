...
class DiskIOStorELT(BaseStoreELT):
    ...
    async def _store_metric_data(self, testing_id: str, metric_name: str, metric_data: Dict[str, Any], data_section: Dict[str, Any]) -> str:
        ...
        columns = ["id", "testing_id", "timestamp", "metric_name", "node_name", 
                  "avg_value", "max_value", "unit", "devices", "promql", "status"]
    ...
    async def _generate_and_store_summary(self, testing_id: str, metrics: Dict[str, Any], data_section: Dict[str, Any]) -> str:
        ...
        for metric_name, metric_data in metrics.items():
            ...
            row_data = (
                ...
                overall_stats.get("min"),
                overall_stats.get("latest"),
        ...
        columns = ["id", "testing_id", "timestamp", "metric_name", "category", "unit",
                  "avg_value", "max_value", "min_value", "latest_value", "data_points",
                  "promql", "node_count", "device_count"]
    ...
    async def get_disk_io_summary(self, testing_id: str) -> Dict[str, Any]:
        ...
        try:
            # Get summary data
            summary_query = f"""
            SELECT metric_name, category, unit, avg_value, max_value, min_value, 
                   latest_value, data_points, node_count, device_count, promql
            ...
            summary_columns = ["metric_name", "category", "unit", "avg_value", "max_value", 
                             "min_value", "latest_value", "data_points", "node_count", "device_count", "promql"]
...
def print_disk_io_summary_tables(summary_data: Dict[str, Any]):
    ...
    header = f"{'Metric Name':<40} {'Category':<15} {'Unit':<8} {'Avg':<10} {'Max':<10} {'Min':<8} {'Latest':<8} {'Data Points':<12} {'Query':<50}"
    print("-" * 160)
    print(header)
    print("-" * 160)
    ...
    for metric in summary_data.get("summary", []):
        ...
        min_val = metric.get("min_value", 0)
        latest_val = metric.get("latest_value", 0)
        data_points = metric.get("data_points", 0)
        promql = metric.get("promql", "")[:48] + "..." if len(metric.get("promql", "")) > 48 else metric.get("promql", "")
        ...
        if "bytes" in unit.lower():
            if avg_val >= 1024*1024:
                ...
                min_str = f"{min_val/(1024*1024):.1f}" if min_val else "0"
                latest_str = f"{latest_val/(1024*1024):.1f}" if latest_val else "0"
            elif avg_val >= 1024:
                avg_str = f"{avg_val/1024:.1f} KB/s"
                max_str = f"{max_val/1024:.1f} KB/s" 
                min_str = f"{min_val/1024:.1f}" if min_val else "0"
                latest_str = f"{latest_val/1024:.1f}" if latest_val else "0"
            else:
                avg_str = f"{avg_val:.1f} B/s"
                max_str = f"{max_val:.1f} B/s"
                min_str = f"{min_val:.1f}" if min_val else "0"
                latest_str = f"{latest_val:.1f}" if latest_val else "0"
        elif "operations" in unit.lower():
            avg_str = f"{avg_val:.3f} IOPS"
            max_str = f"{max_val:.1f} IOPS"
            min_str = f"{min_val:.3f}" if min_val else "0"
            latest_str = f"{latest_val:.1f}" if latest_val else "0"
        else:
        ...
        print(f"{metric_name:<40} {category:<15} {unit:<8} {avg_str:<10} {max_str:<10} {min_str:<8} {latest_str:<8} {data_points:<12} {promql}")
