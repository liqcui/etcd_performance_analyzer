...
class DiskWalFsyncStorELT(BaseStoreELT):
    ...
    async def _create_tables(self):
        ...
        CREATE TABLE IF NOT EXISTS wal_fsync_metrics_overview (
        ...
        CREATE TABLE IF NOT EXISTS wal_fsync_pod_performance_summary (
            id VARCHAR PRIMARY KEY,
            testing_id VARCHAR NOT NULL,
        ...
        create_summary_sql = """
        CREATE TABLE IF NOT EXISTS wal_fsync_summary (
        ...
        # Create indexes
        indexes = [
            "CREATE INDEX IF NOT EXISTS idx_wal_fsync_metrics_overview_testing_id ON wal_fsync_metrics_overview(testing_id)",
            "CREATE INDEX IF NOT EXISTS idx_wal_fsync_pod_summary_testing_id ON wal_fsync_pod_performance_summary(testing_id)",
            "CREATE INDEX IF NOT EXISTS idx_wal_fsync_summary_testing_id ON wal_fsync_summary(testing_id)",
            "CREATE INDEX IF NOT EXISTS idx_wal_fsync_summary_timestamp ON wal_fsync_summary(collection_timestamp)"
        ]
    ...
    def _prepare_overview_operations(self, overview_data: List[Dict[str, Any]]) -> List[tuple]:
        ...
        for record in overview_data:
            insert_sql = """
            INSERT OR REPLACE INTO wal_fsync_metrics_overview 
    ...
    def _prepare_pod_summary_operations(self, pod_summary_data: List[Dict[str, Any]]) -> List[tuple]:
        ...
        for record in pod_summary_data:
            insert_sql = """
            INSERT OR REPLACE INTO wal_fsync_pod_performance_summary 
    ...
    def _prepare_metric_table_operations(self, testing_id: str, metric_name: str, 
        ...
        for pod_name, pod_stats in pod_metrics.items():
            ...
            if "p99" in metric_name:
                avg_val = pod_stats.get("avg_seconds", 0)
                max_val = pod_stats.get("max_seconds", 0) 
                min_val = pod_stats.get("min_seconds", 0)
                latest_val = pod_stats.get("latest_seconds")
            elif "rate" in metric_name:
                if "count" in metric_name:
                    avg_val = pod_stats.get("avg_ops_per_sec", 0) or pod_stats.get("avg_rate_seconds", 0)
                    max_val = pod_stats.get("max_ops_per_sec", 0) or pod_stats.get("max_rate_seconds", 0)
                    min_val = pod_stats.get("min_ops_per_sec", 0) or pod_stats.get("min_rate_seconds", 0)
                    latest_val = pod_stats.get("latest_ops_per_sec") or pod_stats.get("latest_rate_seconds")
                else:
                    avg_val = pod_stats.get("avg_rate_seconds", 0)
                    max_val = pod_stats.get("max_rate_seconds", 0)
                    min_val = pod_stats.get("min_rate_seconds", 0)
                    latest_val = pod_stats.get("latest_rate_seconds")
            elif "sum" in metric_name:
                avg_val = pod_stats.get("avg_sum_seconds", 0)
                max_val = pod_stats.get("max_sum_seconds", 0)
                min_val = pod_stats.get("min_sum_seconds", 0)
                latest_val = pod_stats.get("latest_sum_seconds")
            elif "count" in metric_name:
                avg_val = pod_stats.get("avg_count", 0)
                max_val = pod_stats.get("max_count", 0)
                min_val = pod_stats.get("min_count", 0)
                latest_val = pod_stats.get("latest_count")
            else:
                avg_val = pod_stats.get("avg_value", 0)
                max_val = pod_stats.get("max_value", 0)
                min_val = pod_stats.get("min_value", 0)
                latest_val = pod_stats.get("latest_value")
            
            insert_sql = f"""
            INSERT OR REPLACE INTO {metric_name} 
            (id, testing_id, pod_name, node_name, metric_name, unit, avg_value, 
             max_value, min_value, latest_value, data_points, collection_timestamp)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """
            ...
            operations.append((insert_sql, (
                ...
                min_val, latest_val, pod_stats.get("data_points", 0), timestamp
            )))
    ...
    def _prepare_summary_operation(self, summary_data: Dict[str, Any]) -> tuple:
        ...
        insert_sql = """
        INSERT OR REPLACE INTO wal_fsync_summary 
    ...
    async def generate_summary_report(self, testing_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            FROM wal_fsync_metrics_overview 
            WHERE testing_id = ?
            ORDER BY metric
            """
            ...
            FROM wal_fsync_pod_performance_summary 
            WHERE testing_id = ?
            ORDER BY pod_name
            """
    ...
    async def get_table_info(self) -> Dict[str, List[str]]:
        ...
        try:
            tables = [
                "wal_fsync_metrics_overview",
                "wal_fsync_pod_performance_summary",
                "wal_fsync_summary",
            ] + self.metric_tables
...
async def test_disk_wal_fsync_storage():
    """Test the disk WAL fsync storage functionality"""
    logging.basicConfig(level=logging.INFO)
    
    # Sample data based on the provided JSON
    sample_wal_fsync_data = {
        "status": "success",
        "data": {
            "status": "success",
            "timestamp": "2025-09-18T14:56:05.627241+00:00",
            "duration": "1h",
            ...
            "metrics": {
                "disk_wal_fsync_seconds_duration_p99": {
                    ...
                    "title": "Disk Wal Fsync Seconds Duration P99",
                    "unit": "seconds",
                    "description": "99th percentile WAL fsync duration per etcd pod",
                    "pod_metrics": {
                        "etcd-openshift-qe-019.lab.eng.rdu2.redhat.com": {
                            "avg_seconds": 0.000992,
                            "max_seconds": 0.000998,
                            "min_seconds": 0.00099,
                            "latest_seconds": 0.000991,
                            "data_points": 241
                        },
                        "etcd-openshift-qe-020.lab.eng.rdu2.redhat.com": {
                            "avg_seconds": 0.000992,
                            "max_seconds": 0.000997,
                            "min_seconds": 0.00099,
                            "latest_seconds": 0.000991,
                            "data_points": 241
                        },
                        "etcd-openshift-qe-021.lab.eng.rdu2.redhat.com": {
                            "avg_seconds": 0.001053,
                            "max_seconds": 0.001721,
                            "min_seconds": 0.00099,
                            "latest_seconds": 0.000998,
                            "data_points": 241
                        }
                    },
                    "node_mapping": {
                        "etcd-openshift-qe-019.lab.eng.rdu2.redhat.com": "openshift-qe-019.lab.eng.rdu2.redhat.com",
                        "etcd-openshift-qe-020.lab.eng.rdu2.redhat.com": "openshift-qe-020.lab.eng.rdu2.redhat.com", 
                        "etcd-openshift-qe-021.lab.eng.rdu2.redhat.com": "openshift-qe-021.lab.eng.rdu2.redhat.com"
                    },
                    "total_pods": 3
                },
                "disk_wal_fsync_duration_seconds_count_rate": {
                    "status": "success",
                    "metric": "disk_wal_fsync_duration_seconds_count_rate",
                    "title": "WAL fsync Duration count - rate", 
                    "unit": "count",
                    "description": "Rate of WAL fsync operations per etcd pod",
                    "pod_metrics": {
                        "etcd-openshift-qe-019.lab.eng.rdu2.redhat.com": {
                            "avg_rate_seconds": 5.716874,
                            "max_rate_seconds": 11.266667,
                            "min_rate_seconds": 4.7,
                            "latest_rate_seconds": 5.033333,
                            "data_points": 241
                        },
                        "etcd-openshift-qe-020.lab.eng.rdu2.redhat.com": {
                            "avg_rate_seconds": 5.726833,
                            "max_rate_seconds": 9.333333,
                            "min_rate_seconds": 4.7,
                            "latest_rate_seconds": 5.9,
                            "data_points": 241
                        },
                        "etcd-openshift-qe-021.lab.eng.rdu2.redhat.com": {
                            "avg_rate_seconds": 5.731397,
                            "max_rate_seconds": 11.4,
                            "min_rate_seconds": 4.766667,
                            "latest_rate_seconds": 5.1,
                            "data_points": 241
                        }
                    },
                    "node_mapping": {
                ...
                "disk_wal_fsync_duration_sum": {
                    "status": "success",
                    "metric": "disk_wal_fsync_duration_sum", 
                    "title": "WAL fsync Duration sum",
                    "unit": "seconds",
                    "description": "Cumulative WAL fsync duration per etcd pod",
                    "pod_metrics": {
                        "etcd-openshift-qe-019.lab.eng.rdu2.redhat.com": {
                            "avg_sum_seconds": 297.835371,
                            "max_sum_seconds": 300.311299,
                            "min_sum_seconds": 295.337747,
                            "latest_sum_seconds": 300.311299,
                            "data_points": 241
                        }
                    },
                    "node_mapping": {
                ...
                "disk_wal_fsync_duration_seconds_count": {
                    "status": "success",
                    "metric": "disk_wal_fsync_duration_seconds_count",
    ...
    storage = DiskWalFsyncStorELT("test_wal_fsync.duckdb")
    
    try:
        print(f"Testing WAL Fsync storage with testing_id: {testing_id}")
        
        # Store the data
        result = await storage.store_wal_fsync_metrics(testing_id, sample_wal_fsync_data)
        
        print(f"\nStorage result: {json.dumps(result, indent=2)}")
        
        # Print the formatted summary tables
        if result.get("status") == "success" and "summary_report" in result:
            print_wal_fsync_summary_tables(result["summary_report"])
        
        # Test querying by duration
        print(f"\n\nTesting query by duration...")
        query_result = await storage.query_wal_fsync_data_by_duration("2h")
...
if __name__ == "__main__":
    asyncio.run(test_disk_wal_fsync_storage())