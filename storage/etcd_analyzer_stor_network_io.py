...
class NetworkIOStorELT(BaseStoreELT):
        ...
        try:
            ...
            for metric_name, metric_info in container_data.items():
                ...
                for pod_name, pod_stats in pods.items():
                    ...
                    (id, testing_id, metric_name, pod_name, node_name, avg_value, max_value, unit, query_expression, timestamp)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    """
        ...
        try:
            ...
            for metric_name, metric_info in node_data.items():
                ...
                for node_name, node_stats in nodes.items():
                    ...
                    insert_sql = f"""
                    INSERT OR REPLACE INTO {table_name}
                    (id, testing_id, metric_name, node_name, avg_value, max_value, device_count, unit, query_expression, timestamp)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    """
    ...
    async def _store_cluster_metrics(self, testing_id: str, cluster_data: Dict[str, Any], timestamp: datetime) -> Dict[str, Any]:
        ...
        try:
            ...
            for metric_name, metric_info in cluster_data.items():
                ...
                insert_sql = f"""
                INSERT OR REPLACE INTO {table_name}
                (id, testing_id, metric_name, avg_value, max_value, latest_value, unit, query_expression, timestamp)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                """
    ...
    async def _generate_and_store_summary(self, testing_id: str, timestamp: datetime) -> Dict[str, Any]:
        ...
        try:
            ...
            # Define metric titles mapping
            metric_titles = {
                'network_io_container_network_rx': 'Container Network RX',
                'network_io_container_network_tx': 'Container Network TX',
                'network_io_peer2peer_latency_p99': 'Peer-to-Peer Latency P99',
                'network_io_network_peer_received_bytes': 'Network Peer RX',
                'network_io_network_peer_sent_bytes': 'Network Peer TX',
                'network_io_network_client_grpc_received_bytes': 'gRPC Client RX',
                'network_io_network_client_grpc_sent_bytes': 'gRPC Client TX',
                'network_io_node_network_rx_utilization': 'Node Network RX Utilization',
                'network_io_node_network_tx_utilization': 'Node Network TX Utilization',
                'network_io_node_network_rx_package': 'Node Network RX Packages',
                'network_io_node_network_tx_package': 'Node Network TX Packages',
                'network_io_node_network_rx_drop': 'Node Network RX Drops',
                'network_io_node_network_tx_drop': 'Node Network TX Drops',
            ...
            for metric_name, table_name in self.container_metrics.items():
                try:
                    ...
                    if summary:
                        ...
                        (id, testing_id, metric_name, title, status, min_value, max_value, avg_value, count_value, unit, level, summary_data, timestamp)
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                        """
                        ...
                        params = (
                            ...
                            testing_id,
                            metric_name,
                            metric_titles.get(metric_name, metric_name.replace('_', ' ').title()),
                            summary['status'],
                            summary.get('min_value'),
    ...
    async def get_network_io_summary(self, testing_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            query = f"""
            SELECT metric_name, title, status, min_value, max_value, avg_value, count_value, unit, level
            ...
            for row in results:
                metric_info = {
                    ...
                    "min_value": row[3] if row[3] is not None else "NaN",
                    "max_value": row[4] if row[4] is not None else "NaN", 
                    "avg_value": row[5] if row[5] is not None else "NaN",
                    "count": row[6] if row[6] is not None else "NaN"
                }
    ...
    async def query_network_io_data_by_duration(self, duration: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for metric_name, table_name in self.cluster_metrics.items():
                try:
                    query = f"""
                    SELECT testing_id, avg_value, max_value, latest_value, unit, timestamp
                    FROM {table_name}
                    WHERE timestamp >= ?
...
def print_network_io_summary_tables(summary_data: Dict[str, Any]):
    ...
    try:
        ...
        # Table header
        print(f"{'metric_name':<35} {'status':<10} {'min_value':<15} {'max_value':<15} {'avg_value':<15} {'count':<10}")
        print("-" * 100)
        ...
        for metric in metrics:
            ...
            min_val = metric.get('min_value', 'NaN')
            ...
            min_formatted = format_readable_value(min_val, metric_name)
            max_formatted = format_readable_value(max_val, metric_name)
            avg_formatted = format_readable_value(avg_val, metric_name)
            
            print(f"{metric_name:<35} {status:<10} {min_formatted:<15} {max_formatted:<15} {avg_formatted:<15} {count_formatted:<10}")
